...
class Index extends \Magento\Framework\App\Action\Action implements \Magento\Framework\App\CsrfAwareActionInterface
{
	...
	**/
    ...
    public function __construct(
		...
		\Aqbank\Admin\ERP\Aqbank $aqbank,
    ...
    public function execute()
    {
		...
		if(isset($post['order_id'])){
			$response  = $this->aqbank->send('/order/' . $post['order_id'] , 'GET');
			...
			if(isset($response['success']) && $response['success']) {
				...
				if($order->getId()){
					...
					try {
						...
						if($response['order']['status'] == 'ORDER_PAID' || $response['order']['status'] == 'ORDER_IN_ANALYSIS') {
