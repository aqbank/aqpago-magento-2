    ...
    public function execute(Observer $observer)
    {
        ...
        if ($additionalData->getData('type_payment') == 'credit' ||
            ...
            $payment->setAdditionalInformation(
                'type_payment',
                $additionalData->getData('type_payment')
            );
        } else {
        ...
        if ($additionalData->getData('type_payment') == 'credit' ||
            ...
            $payment->setCcExpMonth(
                preg_replace('/[^\d\-]/', '', $additionalData->getData('one_cc_exp_month'))
            );
            $payment->setCcExpYear(
                preg_replace('/[^\d\-]/', '', $additionalData->getData('one_cc_exp_year'))
            );
            $payment->setCcOwner($additionalData->getData('one_cc_owner'));
            
            $payment->setAdditionalInformation(
                'one_cc_document',
                preg_replace('/[^\d\-]/', '', $additionalData->getData('one_cc_document'))
            );
            $payment->setAdditionalInformation(
                'one_cc_installments',
                (int)$additionalData->getData('one_cc_installments')
            );
            $payment->setAdditionalInformation(
                'one_cc_amount',
                $amountOne
            ...
            $payment->setAdditionalInformation(
                'one_cc_saved',
                $additionalData->getData('one_cc_saved')
            );
            $payment->setAdditionalInformation(
                'one_cc_number_id',
                $additionalData->getData('one_cc_number_id')
            );
        }
        /** Crédito Multiplo **/
        if ($additionalData->getData('type_payment') == 'credit_multiple') {
            $two_cc_number = preg_replace(
                '/[^\d\-]/',
                '',
                $additionalData->getData('two_cc_number')
            );
            $amountTwo = $quote->getGrandTotal() - $amountOne;
            $payment->setAdditionalInformation(
                'two_setCcLast4',
                substr($two_cc_number, -4)
            );
            $payment->setAdditionalInformation(
                'two_setCcNumberEnc',
                $payment->encrypt($two_cc_number)
            );
            $payment->setAdditionalInformation(
                'two_cc_number',
                $two_cc_number
            ...
            $payment->setAdditionalInformation(
                'two_cc_cid',
                $additionalData->getData('two_cc_cid')
            );
            $payment->setAdditionalInformation(
                'two_cc_flag',
                $additionalData->getData('two_cc_flag')
            );
            $payment->setAdditionalInformation(
                'two_cc_exp_month',
                preg_replace('/[^\d\-]/', '', $additionalData->getData('two_cc_exp_month'))
            );
            $payment->setAdditionalInformation(
                'two_cc_exp_year',
                preg_replace('/[^\d\-]/', '', $additionalData->getData('two_cc_exp_year'))
            );
            $payment->setAdditionalInformation(
                'two_cc_owner',
                $additionalData->getData('two_cc_owner')
            );
            $payment->setAdditionalInformation(
                'two_cc_document',
                preg_replace('/[^\d\-]/', '', $additionalData->getData('two_cc_document'))
            );
            $payment->setAdditionalInformation(
                'two_cc_installments',
                (int)$additionalData->getData('two_cc_installments')
            );
            $payment->setAdditionalInformation(
                'two_cc_amount',
                $amountTwo
            ...
            $payment->setAdditionalInformation(
                'two_cc_saved',
                $additionalData->getData('two_cc_saved')
            );
            $payment->setAdditionalInformation(
                'two_cc_number_id',
                $additionalData->getData('two_cc_number_id')
            );
        }
        /** Boleto Multiplo **/
        if ($additionalData->getData('type_payment') == 'ticket_multiple') {
            if (!$additionalData->getData('one_cc_card_saved') != '') {
                $one_cc_number = preg_replace(
                    '/[^\d\-]/',
                    '',
                    $additionalData->getData('one_cc_number')
                );
                $amountOne = number_format($additionalData->getData('one_cc_amount'), 2, '.', '');
                $payment->setCcLast4(substr($one_cc_number, -4));
                $payment->setCcNumberEnc($payment->encrypt($one_cc_number));
            ...
            $payment->setAdditionalInformation(
                'ticket_amount',
                $additionalData->getData('ticket_amount_multiple')
            );
        }
        /** Boleto **/
        if ($additionalData->getData('type_payment') == 'ticket') {
            $payment->setAdditionalInformation(
                'ticket_amount',
                $additionalData->getData('ticket_amount')
            );
        }
    }
}
